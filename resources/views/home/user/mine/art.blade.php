    ...
    <div class="fly-panel fly-panel-user" pad20>
        ...
        <div class="layui-tab layui-tab-brief" lay-filter="mine">
            <ul class="layui-tab-title" id="LAY_mine">
                <li data-type="mine-tpl" data-url="{{route('home.user.mine')}}" lay-id="index" class="layui-this">我的上传（<span>{{$counts['mine_art']}}</span>）</li>
                ...
                <div class="layui-tab-item layui-show" >
                    <ul class="mine-view jie-row">
                        @foreach($items as $item)
                            <li>
                                <a class="jie-title" href="{{route('products.show2',['zm'=>$item->zm,'cate'=>$costCate[$item->parent_id]['zm'],'id'=>$item->id]) }}" target="_blank">{{$item->title}}</a>
                                <i>上传于{{timeAgo($item->created_at)}}</i>  </li>
                        @endforeach
                    </ul>
