...
@if($items)
    @foreach($items as $item)
        <li class="layui-nav-item nav-item">
            <a href="javascript:;" data-id="{{$item['id']}}" data-page="1" data-sort="addtime"  data-action="style">{{$item['title']}}</a>
            @if($item['child'])
            <dl class="layui-nav-child">
                @foreach($item['child'] as $item2)
                <dd><a href="javascript:;" data-id="{{$item2['id']}}" data-page="1" data-sort="addtime" data-action="style">{{$item2['title']}}</a></dd>
                @endforeach
            </dl>
            @endif
        </li>
...
@endif
    ...
    <li class="layui-nav-px tree-op" id="style_sort">排序:
        <span id="style_sort_addtime" title="按时间先后排序" data-type="sort" data-sort="addtime"><i class="fa fa-clock-o"></i>更新时间</span>
